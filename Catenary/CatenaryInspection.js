...
// ------------------------ HOW TO USE IT --------------------------------------------
// (!) Make sure that your 4 polylines, corresponding to the rails, the axis and the catenary, are visible.
...
// 2. Select the different polylines needed for catenary inspection: catenary, left rail , right rail and then the track axis.
// 3. The catenary position is computed for each individual point on the electric line.
// 4. Choose the file path where to save the *.csv file containing the catenary data.
...
/**
* @typedef {Object} Axes
* @property {SMultiline} ElectricLine The selected catenay
...
function SelectAxes() {
    ...
    if (visibleLines.length >= 4) {
        print("Click the catenary");
        var electricLineClicked = SMultiline.FromClick();
        while(electricLineClicked.ErrorCode != 0)
        {	
            print("Failed to select the multiline. Please click the catenary");
            electricLineClicked = SMultiline.FromClick();
        }
        electricLine = electricLineClicked.Multi;
    ...
    if (electricLine.GetLength() <= 0 ||leftRail.GetLength() <= 0 || rightRail.GetLength() <= 0 || trackAxis.GetLength() <= 0)
...
function main() {
    ...
    // Step 2: Select elements for catenary script computation: left rail, right rail and track axis
    var multilinesSelected = SelectAxes();

	var electricLine= multilinesSelected.ElectricLine;
